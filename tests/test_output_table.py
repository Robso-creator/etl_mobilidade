...
def test_send_to_db(sample_dataframe, mock_engine, mock_inspect, mock_logger):
    ...
    send_to_db(sample_dataframe, TableClass, chunk_size=2)
...
def test_send_to_db_table_not_exist(sample_dataframe, mock_inspect):
    mock_inspector = MagicMock()
...
def test_send_to_db_sqlalchemy_error(sample_dataframe, mock_inspect, mock_logger):
    mock_inspector = MagicMock()
    mock_inspector.get_table_names.return_value = ['test_table']
    mock_inspector.get_columns.return_value = [{'name': 'col1'}, {'name': 'col2'}]
    mock_inspect.return_value = mock_inspector
